    ...
    <header className="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {links.map((link) => (
            <Link
              key={link.title}
              href={link.url}
              className="text-[#1A1A1A] hover:opacity-70 font-medium text-sm transition-colors"
            >
              {link.title}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4">
          <Link href="/search" className="text-[#1A1A1A] hover:opacity-70">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </Link>
          <Link href="/wishlist" className="text-[#1A1A1A] hover:opacity-70">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            ...
            className="relative text-[#1A1A1A] hover:opacity-70"
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-[var(--color-accent)] text-white text-xs font-bold rounded-full w-4 h-4 flex items-center justify-center">
                {cartCount > 9 ? "9+" : cartCount}
              </span>
            )}
          </button>
          <Link href={isAuthenticated ? "/account/profile" : "/account/login"} className="text-[#1A1A1A] hover:opacity-70">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">

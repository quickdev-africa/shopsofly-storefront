...
function BundlePickerModal({ bundles, initialBundle, productName, productImages, onConfirm, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.88)" }}>
      <div className="w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden" style={{ maxHeight: "90vh", overflowY: "auto" }}>
        {/* Header */}
        <div className="bg-[#F97316] px-5 py-4 flex items-center justify-between sticky top-0">
          <div>
            <p className="text-white font-black text-base">Choose Your Bundle</p>
            <p className="text-orange-100 text-xs mt-0.5">Flash sale prices — pick your pack</p>
          </div>
          <button onClick={onClose} className="text-white text-2xl leading-none font-bold">&times;</button>
        </div>

        <div className="p-4">
          {/* Product preview */}
          <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-orange-50 border border-orange-100">
            {productImages[0] && <img src={productImages[0]} alt={productName} className="w-14 h-14 rounded-lg object-cover flex-shrink-0" />}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-gray-900 truncate">{productName}</p>
              <p className="text-xs text-gray-500 mt-0.5">Select quantity below</p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-4">
            {bundles.map((b, i) => (
              <div key={i} onClick={() => setSelected(i)}
                className="relative rounded-xl cursor-pointer flex items-center gap-3 px-3 py-3 transition-all"
                ...
                {b.tag && (
                  <div className="absolute px-2 py-0.5 rounded text-white font-bold"
                    style={{ top: "-8px", left: "10px", background: "#F97316", fontSize: "9px", letterSpacing: "1px" }}>
                    {b.tag}
                  </div>
                )}
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold" style={{ color: selected === i ? "#fff" : "#111827" }}>{b.label}</p>
                  <p className="text-xs mt-0.5" style={{ color: selected === i ? "#fdba74" : "#9CA3AF" }}>{fmt(b.perUnit)}/bottle</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold" style={{ color: "#F97316" }}>{fmt(b.price)}</p>
                  <p className="text-xs line-through" style={{ color: selected === i ? "#9CA3AF" : "#D1D5DB" }}>{fmt(b.originalPrice)}</p>
                  <p className="text-xs font-bold text-green-600">Save {fmt(b.savings)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Selected summary */}
          <div className="rounded-xl p-3 mb-4 flex items-center justify-between bg-orange-50 border border-orange-200">
            <div>
              <p className="text-xs text-gray-500">Selected</p>
              <p className="text-sm font-bold text-gray-900">{current.label}</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-black text-orange-600">{fmt(current.price)}</p>
              <p className="text-xs font-semibold text-green-600">Save {fmt(current.savings)}</p>
            </div>
          </div>

          <button onClick={() => onConfirm(selected)}
            className="w-full bg-[#F97316] hover:bg-orange-600 text-white font-black py-4 rounded-xl text-base transition-all">
            Add to Cart &amp; Checkout →
          </button>
          <p className="text-xs text-gray-400 text-center mt-3">🔒 Secure checkout · Pay on delivery available</p>
        </div>
      </div>
    </div>
  );
      ...
      {[{ v: pad(h), l: "Hours" }, { v: pad(m), l: "Mins" }, { v: pad(s), l: "Secs" }].map((u, i) => (
        <React.Fragment key={i}>
          <div className="text-center px-3 py-2 rounded-xl" style={{ background: bg }}>
            ...
            <div className="text-xs mt-1 text-gray-500">{u.l}</div>
          </div>
...
function CTABtn({ text, onClick }: { text: string; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="w-full font-black rounded-xl py-4 text-base text-white transition-all hover:opacity-90 shadow-lg"
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "#1A1A1A", border: "1px solid #2D2D2D" }}>
      ...
      {caption && (
        <div className="px-4 py-3">
          ...
          {location && <p className="text-xs mt-1 text-gray-500">{location}</p>}
        </div>
      )}
    </div>
  );
...
export default function FlashSaleTemplate({ page, store }: { page: any; store: any }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0A0A0A", color: "#fff" }}>
      ...
      {/* ── HERO ───────────────────────────────────────────────── */}
      <section className="relative py-10 overflow-hidden text-center" style={{ background: "#0A0A0A", minHeight: "420px" }}>
        {s.hero_image_url && (
          <>
        ...
        <div className="relative max-w-2xl mx-auto px-4">
          <div className="inline-block px-4 py-1.5 rounded-full text-xs font-bold mb-4 tracking-widest uppercase" style={{ background: "#F97316" }}>
            ⚡ {s.badge_text || "FLASH SALE — LIMITED TIME ONLY"}
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-4 leading-tight">{s.hero_headline || productName}</h1>
          <p className="text-gray-300 text-sm mb-6 leading-relaxed">{s.hero_subheadline || "Buy more and save up to 40%. Nigeria's most powerful stain-fighting soap."}</p>
          {s.countdown_end && (
            <div className="max-w-xs mx-auto mb-4 rounded-xl px-4 py-2" style={{ background: "#1A1A1A", border: "1.5px solid #F97316" }}>
          ...
          {s.stock_count && <p className="text-xs font-bold mb-4 text-red-400">🔥 Only {s.stock_count} packs left at this price!</p>}
          <div className="flex items-center justify-center gap-2 mb-6">
            <span className="text-sm font-bold text-[#F97316]">★★★★★</span>
            <span className="text-xs text-gray-400">{s.rating || "4.9"} · {s.review_count || "1,240+"} verified reviews</span>
          </div>
          <CTABtn text={ctaText} onClick={openBundlePicker} />
          {theme.whatsapp_number && (
            <div className="mt-4">
              <a href={`https://wa.me/${theme.whatsapp_number.replace(/\D/g, "")}`} className="text-green-400 text-sm font-medium">
      ...
      {/* ── DELIVERY BAR ───────────────────────────────────────── */}
      <div className="py-3 text-center text-sm font-bold text-white" style={{ background: "#F97316" }}>
      ...
      <section className="py-10" style={{ background: "#111" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.showcase_title || productName}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.showcase_subtitle || "Effective for hand wash and machine wash · All fabric types"}</p>
          ...
          {productImages.length > 0 && (
            <>
              ...
              {productImages.length > 1 && (
                <div className="grid gap-2 mb-5" style={{ gridTemplateColumns: `repeat(${Math.min(productImages.length, 4)}, 1fr)` }}>
            ...
            {(s.showcase_features || "Removes tough stains instantly\nWhiter whites, brighter colours\nGentle on all fabric types\nSaves water — less rinsing\nHand wash and machine wash\nLong lasting formula")
              .split("\n").filter(Boolean).map((f: string, i: number) => (
                <div key={i} className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "#1A1A1A", border: "1px solid #2D2D2D" }}>
                  <div className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center" style={{ background: "#4ade8020", border: "1.5px solid #4ade80" }}>
      ...
      <section className="py-10" style={{ background: "#0D0D0D" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.bundle_title || "Choose Your Bundle & Save"}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.bundle_subtitle || "Flash sale prices — discount automatically applied at checkout"}</p>

          <div className="flex flex-col gap-2 mb-5">
            {bundles.map((b, i) => (
              <div key={i} onClick={() => setSelectedBundle(i)}
                className="relative rounded-xl cursor-pointer flex items-center gap-3 px-3.5 py-3 transition-all"
                style={{ background: selectedBundle === i ? "#1A0800" : "#1A1A1A", border: selectedBundle === i ? "2px solid #F97316" : "1.5px solid #2D2D2D" }}>
                {b.tag && (
                  <div className="absolute px-2 py-0.5 rounded font-bold text-white"
                    style={{ top: "-8px", left: "10px", background: "#F97316", fontSize: "9px", letterSpacing: "1px" }}>
                    {b.tag}
                  </div>
                )}
                <div className="w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center"
                  ...
                  {selectedBundle === i && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-semibold text-white">{b.label}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{fmt(b.perUnit)}/bottle</p>
                </div>
                <div className="text-right">
                  <p className="text-base font-bold text-[#F97316]">{fmt(b.price)}</p>
          ...
          <div className="rounded-xl p-4 mb-4 flex items-center gap-3" style={{ background: "#1A1A1A", border: "1px solid #2D2D2D" }}>
            ...
            <div className="text-right">
              <p className="text-lg font-black text-[#F97316]">{fmt(current.price)}</p>
              <p className="text-xs font-bold text-green-500">Save {fmt(current.savings)}</p>
            </div>
          </div>

          <CTABtn text={ctaText} onClick={openBundlePicker} />
          <p className="text-center text-xs mt-3 text-gray-600">🔒 Secure checkout · Pay on delivery available · 30-day guarantee</p>
        </div>
      </section>
      ...
      <section className="py-10" style={{ background: "#111" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.benefits_title || "Why Nigerians Love Double Plus"}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.benefits_subtitle || "Real results in every wash"}</p>
          <div className="grid grid-cols-2 gap-3 mb-6">
            {(s.benefits_items || "Removes Tough Stains|Grease, oil, Ankara stains gone in one wash\nWhiter Whites|Brilliant whites and brighter colours every time\nGentle on Fabric|Safe for all fabrics including delicates\nSaves Water|Less rinsing needed\nHand or Machine|Works perfectly in both\nLong Lasting|One bottle goes 3x further")
              .split("\n").filter(Boolean).map((item: string, i: number) => {
                ...
                return (
                  <div key={i} className="rounded-xl p-3 text-center" style={{ background: "#1A1A1A", border: "1px solid #2D2D2D" }}>
                    ...
                    <p className="text-xs text-gray-500" style={{ lineHeight: "1.4" }}>{desc?.trim()}</p>
                  </div>
                );
              })}
          </div>
      ...
      <section className="py-10" style={{ background: "#0A0A0A" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.video1_title || "See It in Action"}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.video1_subtitle || "Watch how it tackles Nigeria's toughest stains"}</p>
      ...
      <section className="py-10" style={{ background: "#111" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.testimonials_title || "What Our Customers Are Saying"}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.testimonials_subtitle || "Real reviews from real Nigerian households"}</p>
          {[
            ...
            <div key={i} className="rounded-xl p-4 mb-3" style={{ background: "#1A1A1A", border: "1px solid #2D2D2D" }}>
              <div className="text-sm mb-2 text-[#F97316]">★★★★★</div>
              <p className="text-sm italic mb-3 text-gray-300" style={{ lineHeight: "1.6" }}>&ldquo;{t.text}&rdquo;</p>
              <p className="text-xs font-bold text-gray-500">{t.author}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-10" style={{ background: "#0A0A0A" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-1">{s.video2_title || "Real Customers Share Their Experience"}</h2>
          <p className="text-xs text-gray-400 text-center mb-5">{s.video2_subtitle || "Unscripted testimonials from Nigerian households"}</p>
          <YTEmbed url={s.video2_url || DEFAULT_VIDEO_URL} caption={s.video2_caption} location={s.video2_location} />
          <div className="mt-5"><CTABtn text={ctaText3} onClick={openBundlePicker} /></div>
        </div>
      </section>
      ...
      <section className="py-10" style={{ background: "#111" }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-xl font-black text-white text-center mb-6">{s.steps_title || "How to Order — 3 Simple Steps"}</h2>
          <div className="flex flex-col gap-4 mb-6">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center font-black text-white text-sm" style={{ background: "#F97316" }}>
                  {step.n}
                </div>
                <p className="text-sm pt-1.5 text-gray-300" style={{ lineHeight: "1.5" }}>{step.t}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── FINAL CTA ───────────────────────────────────────────── */}
      <section className="py-12" style={{ background: "#0D0D0D" }}>
        <div className="max-w-2xl mx-auto px-4">
          ...
          <p className="text-xs text-gray-400 text-center mb-5">{s.final_cta_subtitle || "Prices go back to normal when the timer hits zero"}</p>
          {s.countdown_end && (
            <div className="max-w-xs mx-auto mb-5 rounded-xl px-4 py-2" style={{ background: "#1A1A1A", border: "1.5px solid #F97316" }}>
              <FlashCountdown endDate={s.countdown_end} />
            </div>
          )}
          ...
          <p className="text-center text-xs mt-3 text-gray-600">🔒 Secure checkout · Paystack · Pay on delivery available</p>
          {theme.whatsapp_number && (

...
function LeadPopup({ onClose, slug, ctaText, productPrice, dark }: {
  ...
  const inputCls = dark
    ? "w-full px-4 py-3 rounded-xl text-sm outline-none bg-[#1A1A1A] border border-gray-700 text-white placeholder-gray-500 focus:border-[#F97316]"
      ...
      <div className="w-full max-w-md shadow-2xl rounded-2xl" style={{ backgroundColor: "#FFFFFF" }}>
        {success ? (
          <div className="text-center py-6">
            ...
            <button onClick={onClose} className="bg-[#F97316] text-white px-6 py-3 rounded-xl font-semibold w-full">Close</button>
          </div>
        ) : (
          <>
            <div className="p-5">
...
function CTAButton({ text, onClick, size = "lg" }: { text: string; onClick: () => void; size?: "sm" | "lg" }) {
  return (
    <button onClick={onClick} className={`inline-block bg-[#F97316] hover:bg-orange-500 text-white font-bold rounded-xl transition-all shadow-lg hover:shadow-orange-500/30 hover:scale-105 ${size === "lg" ? "px-10 py-5 text-lg" : "px-6 py-3 text-sm"}`}>
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: pageBg }}>
      ...
      {(s.rating || s.customer_count || s.orders_today) && (
        <section className="py-8 px-4" style={{ backgroundColor: "#F97316" }}>
        ...
        const sectionMap: Record<string, React.ReactNode> = {
          ...
          benefits2: parseBenefits(s.benefits2_items).length > 0 ? (
            <BenefitsSection key="benefits2" heading={s.benefits2_heading} items={parseBenefits(s.benefits2_items)}
              bg={isDark ? "#0A0A0A" : "#1A1A1A"} fg="#FFFFFF" checkColor="#F97316"
              onClick={() => setShowPopup(true)} ctaText={ctaText} />
          ) : null,
          ...
          benefits3: parseBenefits(s.benefits3_items).length > 0 ? (
            <BenefitsSection key="benefits3" heading={s.benefits3_heading} items={parseBenefits(s.benefits3_items)}
              bg={isDark ? "#111111" : "#FFF7ED"} fg={primaryText} checkColor="#F97316"
              onClick={() => setShowPopup(true)} ctaText={ctaText} />
          ) : null,
